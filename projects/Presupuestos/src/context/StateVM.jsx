...
const StateVM = ({ children }) => {
  ...
  const addVM = (newId) => {
    const newVM = <VirtualMachine key={newId} id={newId} vm={listaVM.length} />;
    setListaVM([...listaVM, newVM]);
  };

  const deleteVM = (num) => {
    setListaVM((prevLista) =>
      prevLista.filter((component) => component.props.id !== num)
    );
  };
  ...
  return (
    <vmContext.Provider value={{ listaVM, addVM, deleteVM, getPositionById }}>
      {children}
    </vmContext.Provider>
  );
};
